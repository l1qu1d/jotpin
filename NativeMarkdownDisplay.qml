...
Item {
  ...
  property var inlineCodeSourceRanges: []
  ...
  // Readiness for the first reveal, not for hiding subsequent edits.
  readonly property bool initialLayoutReady: layoutReady &&
    settledRequestId === parseRequestId && !parsePending && !parseInFlight &&
    codeHighlightPendingCount === 0 && !codeHighlightDispatchPending &&
    layoutSourceText === sourceText
  readonly property bool imagesReady: {
    for (var index = 0; index < images.length; index++) {
      if (imageLoadStateAt(index) === "loading") return false
    }
    return true
  }
  ...
  function tryApplyOptimisticPlainEdit(nextSourceValue) {
    ...
    // Choose the known semantic color instead of TextEdit's mutable cursor
    // format, which can carry a default or unrelated selection color.
    var insertionColor = foreground
    var insertionLineStart = previousSource.lastIndexOf("\n", Math.max(0, prefix - 1)) + 1
    if (/^[ \t]*>/.test(previousSource.slice(insertionLineStart, prefix)))
      insertionColor = cssColor(Qt.rgba(foreground.r, foreground.g, foreground.b, 0.78))
    for (var colorIndex = 0; colorIndex < inlineCodeSourceRanges.length; colorIndex++) {
      var colorRange = inlineCodeSourceRanges[colorIndex]
      if (prefix > colorRange.start && previousEnd < colorRange.end) {
        insertionColor = accent
        break
      }
    }
    ...
    if (documentInserted.length > 0 &&
        !EditorModel.fencedCodeContext(previousSource, prefix)) {
      nativeDocument.select(documentStart, documentStart + documentInserted.length)
      nativeDocument.cursorSelection.color = insertionColor
      nativeDocument.deselect()
    }
    ...
    inlineCodeSourceRanges = inlineCodeSourceRanges.map(function(range) {
      return {start: range.start >= previousEnd ? range.start + sourceDelta : range.start,
        end: range.end > prefix ? range.end + sourceDelta : range.end}
    })
  ...
  function characterFormatForDocumentForTests(documentPositionValue) {
    ...
    var result = {
      ...
      color: String(nativeDocument.cursorSelection.color)
    }
    nativeDocument.deselect()
    return result
  }
  ...
  function imageTagWithWidth(tagValue, widthValue) {
    ...
    var width = Math.max(1, Math.round(Number(widthValue) || 0))
  ...
  function effectiveImageSize(index) {
    var model = images[index] || {}
    var natural = imageSizeCache[linkSourceUrl(model.url)]
    if (!natural || natural.width <= 0 || natural.height <= 0) return null
    var requested = index === previewImageIndex && previewImageWidth > 0
      ? previewImageWidth : Number(model.width) || natural.width
    var imageWidth = Math.min(Math.max(48, width - horizontalPadding * 2), requested)
    return {width: imageWidth, height: imageWidth * natural.height / natural.width}
  }
  ...
  function htmlWithEffectiveImageWidths(value) {
    ...
    while ((match = expression.exec(html)) !== null) {
      ...
      var effective = effectiveImageSize(index)
      result += effective || requested > 0
        ? imageTagWithWidth(match[0], effective ? effective.width : Math.min(maximumWidth, requested))
        : match[0]
  ...
  function rebuildMappings(requestId) {
    ...
    inlineCodeSourceRanges = segments.filter(function(segment) {
      return segment.type === "inlineCode"
    }).map(function(segment) { return {start: segment.start, end: segment.end} })
  ...
  function rebuildImageRects() {
    ...
    for (var index = 0; index < models.length; index++) {
      ...
      var size = effectiveImageSize(index)
      var sameRow = Math.abs(Number(after.y) - Number(start.y)) <= 0.75
      // A full-width image can put the following caret on the next row.
      // Its intrinsic aspect ratio still gives us its actual object bounds.
      var imageWidth = size ? size.width : sameRow ? Number(after.x) - Number(start.x) : 0
      var imageHeight = size ? size.height : Number(start.height) - Math.max(1, bodyPixelSize * 0.3)
    ...
    delegate: Image {
      ...
      function reportStatus() {
        if (status === Image.Ready && sourceSize.width > 0 && sourceSize.height > 0) {
          var sizes = Object.assign({}, root.imageSizeCache)
          sizes[root.linkSourceUrl(ownerUrl)] = {width: sourceSize.width, height: sourceSize.height}
          root.imageSizeCache = sizes
        }
  ...
  Rectangle {
    ...
    objectName: "imageSelectionFrame"
    ...
    Repeater {
      ...
      delegate: Rectangle {
        required property var modelData
        objectName: "imageHandle_" + modelData.name
        ...
        color: Qt.rgba(root.accent.r, root.accent.g, root.accent.b, 1)
  ...
  WorkerScript {
    ...
    onMessage: function(message) {
      ...
      // A debounce timer can fire while this same revision is in flight and
      // mark it pending again. Its current reply satisfies that request too.
      root.parsePending = false
      parseDelayTimer.stop()
