...
function applyImageMetadata(node, images, state) {
  if (!node || typeof node !== "object") return
  ...
  for (const child of Array.isArray(node.children) ? node.children : [])
    applyImageMetadata(child, images, state)
}
  ...
  function flattenedListChildren(list, depth) {
    ...
    for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
      ...
      result.push({
        type: "element",
        tagName: "p",
        properties: {className: [task
          ? "jotpin-task-list-item" : "jotpin-list-item"]},
        children: [{type: "text", value: marker}, ...parts.content],
