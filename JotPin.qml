...
Item {
  ...
  function loadPresentationSettings(rawValue) {
    ...
    if (hadPendingMode || hadPendingSidePlacement || hadPendingSessionPath ||
        hadPendingNotesDirectory || hadPendingFileTabRows ||
  ...
  function writeNote() {
    ...
    var savedCursorPosition = Number(editor.cursorPosition)
    var savedSelectionStart = Number(editor.selectionStart)
    var savedSelectionEnd = Number(editor.selectionEnd)
    root.pendingSave = false
    ...
    Qt.callLater(function() {
      if (!root.opened || root.notePath !== savedPath ||
          root.loadingFromFile || String(editor.text || "") !== savedSource) {
        return
      }
      if (savedSelectionStart !== savedSelectionEnd) {
        editor.select(
          Math.max(0, Math.min(savedSelectionStart, editor.length)),
          Math.max(0, Math.min(savedSelectionEnd, editor.length)))
      } else if (isFinite(savedCursorPosition)) {
        editor.cursorPosition = Math.max(0,
          Math.min(savedCursorPosition, editor.length))
      }
    })
  }
  ...
  FileView {
    ...
    onSaved: {
      root.presentationSettingsWriteInFlight = false
      if (root.presentationSettingsWritePending)
        root.writePresentationSettings()
    }
  ...
  FocusScope {
    ...
    onActiveFocusChanged: {
      ...
      BorderSurface {
        id: card
        width: root.sideMode
          ? Math.max(Style.space(360), parent.width -
          ...
          Item {
            ...
            Rectangle {
              ...
              ScrollView {
                ...
                Text {
                  width: parent.width
                  text: "English (US) spellcheck is included and runs locally. " +
                    "Code, inline code, URLs, and link destinations are skipped."
                  color: Qt.darker(root.foreground, 1.35)
                  font.family: Style.font.family
                  font.pixelSize: Style.font.bodySmall
                  wrapMode: Text.WordWrap
                ...
                Row {
                  ...
                  JotPinButton {
                    text: "On"
                    selected: root.spellcheckEnabled
                    focusable: true
                    tooltipText: "Underline misspelled words and offer corrections"
                    onClicked: root.setSpellcheckEnabled(true)
                  }
                  ...
                  JotPinButton {
                    ...
                    selected: !root.spellcheckEnabled
                    focusable: true
                    tooltipText: "Disable spellcheck"
                    onClicked: root.setSpellcheckEnabled(false)
                  }
                }
