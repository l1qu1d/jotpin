...
function listBackspace(sourceValue, value, selectionStart, selectionEnd,
  ...
  if (collapseEmptyItem === true && /^\s*$/.test(list[4]) &&
      cursor === bounds.lineEnd) {
    // Preview hides list syntax, so deleting its marker one source character
    // at a time creates phantom caret stops. Remove the complete empty row and
    // land on the previous visible item end. Raw mode does not request this.
    var removeStart = bounds.lineStart
    var removeEnd = bounds.lineEnd
    var previousVisibleEnd = bounds.lineStart
    if (previousVisibleEnd > 0) {
      previousVisibleEnd--
      if (previousVisibleEnd > 0 &&
          source.charAt(previousVisibleEnd - 1) === "\r")
        previousVisibleEnd--
    }
    if (removeEnd < source.length) {
      removeEnd += bounds.lineBreak.length
    } else if (removeStart > 0) {
      removeStart = previousVisibleEnd
    }
    return {
      handled: true,
      source: source.slice(0, removeStart) + source.slice(removeEnd),
      cursor: previousVisibleEnd
    }
  }
