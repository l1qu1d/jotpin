...
cp -- "$ROOT_DIR/tests/helpers/blocked_recovery_rm.sh" "$TEST_BIN_DIR/rm"
chmod 700 "$TEST_BIN_DIR/ln" "$TEST_BIN_DIR/mv" "$TEST_BIN_DIR/rm"
...
run_stage() {
  ...
  if env -u QT_QPA_PLATFORMTHEME -u WAYLAND_DISPLAY -u DISPLAY \
      ...
      JOTPIN_TEST_RECOVERY_RELEASE="$TEST_TMP_DIR/recovery-release" \
    ...
    local exit_status=$?
    cat -- "$output_path" >&2
    fail "offscreen persistence stage $mode failed (exit $exit_status)"
  fi
...
if [[ "${1:-}" == "external-change" ]]; then
  run_stage external-change
  exit 0
fi

if [[ "${1:-}" == "editor-commands" ]]; then
  run_stage editor-commands
  exit 0
fi

if [[ "${1:-}" == "terminal-reentry" ]]; then
  run_stage terminal-reentry
  exit 0
fi

run_spellcheck_close() {
  local note_before
  note_before="$(<"$TEST_NOTE_PATH")"
  run_stage spellcheck-close
  [[ "$(<"$TEST_NOTE_PATH")" == "$note_before" ]] || \
    fail "spellcheck close invalidation changed saved note bytes"
}

if [[ "${1:-}" == "spellcheck-close" ]]; then
  run_spellcheck_close
  exit 0
fi

run_recovery_cleanup_race() {
  local snapshot_path
  run_stage recovery-cleanup-race
  snapshot_path="$(sed -n 's/.*PERSIST_RECOVERY_PATH: //p' \
    "$TEST_TMP_DIR/recovery-cleanup-race.log" | tail -n 1)"
  [[ "$snapshot_path" == "$TEST_HOME/.local/state/jotpin/recovery/"* ]] || \
    fail "cleanup race did not report an isolated snapshot"
  [[ "$(jq -r '.source' "$snapshot_path")" == "cleanup-after" ]] || \
    fail "old recovery cleanup deleted the newer snapshot"
  [[ "$(<"$TEST_NOTE_PATH")" == "cleanup-before" ]] || \
    fail "cleanup race saved the unsaved edit to the note"
  rm -- "$snapshot_path" "$TEST_TMP_DIR/recovery-release"
  cp -- "$ROOT_DIR/tests/fixtures/persistence-base.md" "$TEST_NOTE_PATH"
  rm -f -- "$TEST_HOME/.local/state/jotpin/settings.json" \
    "$TEST_HOME/.local/state/jotpin/editor-states.json"
  run_stage recovery-cleanup-save
  [[ ! -e "$snapshot_path" && "$(<"$TEST_NOTE_PATH")" == "cleanup-after" ]] || \
    fail "saving during cleanup recreated a stale snapshot or lost saved bytes"
  rm -- "$TEST_TMP_DIR/recovery-release"
  cp -- "$ROOT_DIR/tests/fixtures/persistence-base.md" "$TEST_NOTE_PATH"
  rm -f -- "$TEST_HOME/.local/state/jotpin/settings.json" \
    "$TEST_HOME/.local/state/jotpin/editor-states.json"
  printf '%s\n' 'PASS: old snapshot cleanup cannot delete a newer unsaved edit or recreate a saved one'
}

if [[ "${1:-}" == "recovery-cleanup-race" ]]; then
  run_recovery_cleanup_race
  exit 0
fi

run_close_completion() {
  run_stage close-save-completion
  [[ "$(<"$TEST_NOTE_PATH")" == "close completion saved content" ]] || \
    fail "closing a saving tab lost its final source"
  cmp -s "$TEST_OPEN_NOTE" "$ROOT_DIR/tests/fixtures/persistence-base.md" || \
    fail "closing a saving tab changed the replacement note"
  cp -- "$ROOT_DIR/tests/fixtures/persistence-base.md" "$TEST_NOTE_PATH"
  rm -f -- "$TEST_HOME/.local/state/jotpin/settings.json" \
    "$TEST_HOME/.local/state/jotpin/editor-states.json"
  run_stage close-load-completion
  cmp -s "$TEST_NOTE_PATH" "$ROOT_DIR/tests/fixtures/persistence-base.md" || \
    fail "closing a loading tab changed the original note"
  cmp -s "$TEST_OPEN_NOTE" "$ROOT_DIR/tests/fixtures/persistence-base.md" || \
    fail "closing a loading tab changed its source"
  rm -f -- "$TEST_HOME/.local/state/jotpin/settings.json" \
    "$TEST_HOME/.local/state/jotpin/editor-states.json"
  printf '%s\n' 'PASS: closing loading and saving tabs preserves bytes and completes replacement reads'
}

if [[ "${1:-}" == "close-completion" ]]; then
  run_close_completion
  exit 0
fi

run_queued_recovery() {
  local original_note queued_recovery_path
  original_note="$(<"$TEST_NOTE_PATH")"
  run_stage recovery-queued
  queued_recovery_path="$(sed -n 's/.*PERSIST_RECOVERY_PATH: //p' \
    "$TEST_TMP_DIR/recovery-queued.log" | tail -n 1)"
  [[ "$queued_recovery_path" == "$TEST_HOME/"* ]] || \
    fail "queued recovery snapshot escaped the isolated home"
  [[ "$(jq -r '.source' "$queued_recovery_path")" == "recovery-final" ]] || \
    fail "queued recovery did not persist the latest source"
  [[ "$(<"$TEST_NOTE_PATH")" == "$original_note" ]] || \
    fail "recovery snapshots changed the saved note"
  rm -- "$queued_recovery_path"
  mkdir -- "$queued_recovery_path"
  run_stage recovery-failure
  [[ -d "$queued_recovery_path" && "$(<"$TEST_NOTE_PATH")" == "$original_note" ]] || \
    fail "failed recovery changed its destination or the saved note"
  rmdir -- "$queued_recovery_path"
  printf '%s\n' 'PASS: queued recovery saved the latest snapshot without saving the note'
}

run_queued_dictionary() {
  local personal_dictionary_path="$TEST_HOME/.local/state/jotpin/personal-dictionary.json"
  run_stage dictionary-queued
  jq -e --arg first "jotpinalpha" --arg second "jotpinbeta" \
    '(.words | index($first) != null) and
     (.words | index($second) != null)' \
    "$personal_dictionary_path" >/dev/null || \
    fail "queued personal dictionary save did not persist both words"
  printf '%s\n' 'PASS: queued personal dictionary saves persisted both words'
}

if [[ "${1:-}" == "recovery-queued" ]]; then
  run_queued_recovery
  exit 0
fi

if [[ "${1:-}" == "dictionary-queued" ]]; then
  run_queued_dictionary
  exit 0
fi
...
run_tab_state() {
  ...
  jq -e --arg first "$TEST_TAB_STATE_FIRST" --arg second "$TEST_TAB_STATE_SECOND" \
    '(.version == 1) and
     ...
     ([.files[] | select(.path == $first) | .selectionStart][0] == 300) and
   ([.files[] | select(.path == $first) | .selectionEnd][0] == 310) and
   ([.files[] | select(.path == $first) | .liveScrollY][0] == 120) and
...
run_settings() {
  local session_settings_path="$TEST_HOME/.local/state/jotpin/settings.json"
  run_stage settings
  [[ -d "$TEST_SETTINGS_DIR" ]] || \
  ...
  [[ "$(jq -r '.editorTextScale' "$session_settings_path")" == "150" ]] || \
    fail "editor text size was not persisted"
  run_stage settings-restore
  run_stage settings-queued
  [[ "$(jq -r '.editorTextScale' "$session_settings_path")" == "150" ]] || \
    fail "queued settings save did not persist the final text size"
  printf '%s\n' 'PASS: queued settings saves persisted the final value'
}
...
jq -e --arg session_path "$session_path" \
  '.files | any(.path == $session_path)' \
  "$TEST_HOME/.local/state/jotpin/editor-states.json" >/dev/null || \
  fail "closing JotPin did not persist the generated tab editor state"
