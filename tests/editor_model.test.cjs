...
equal(
  model.listBackspace("text\n- ", 7, 7, 7, true),
  { handled: true, source: "text\n", cursor: 5 },
  "Preview Backspace removes the bullet and preserves its row"
)
equal(
  model.listBackspace("- test\n- \n- next", 9, 9, 9, true),
  { handled: true, source: "- test\n\n- next", cursor: 7 },
  "Preview Backspace keeps the empty row before another item"
)
equal(
  model.listBackspace("text\n-   ", 9, 9, 9),
  { handled: false, source: "text\n-   ", cursor: 9 },
...
for (const prefix of ["- ", "* ", "1. ", "2) ", "- [ ] ", "- [x] ", "## ", "> ", "> > ", "  - "]) {
  for (const content of ["", "Remaining text"]) {
    const source = "Before\n" + prefix + content + "\nAfter";
    const cursor = 7 + prefix.length;
    equal(model.formattingPrefixBackspace(source, cursor, cursor, cursor),
      {handled: true, source: "Before\n" + content + "\nAfter", cursor: 7},
      "Backspace removes formatting while retaining its source row: " + prefix);
  }
}
equal(model.formattingPrefixBackspace("```\n- \n```", 6, 6, 6).handled,
  false, "Backspace does not strip literal prefixes inside code");
