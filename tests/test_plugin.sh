...
assert_contains "$ROOT_DIR/README.md" \
  'JotPin never unbinds or replaces it' \
...
assert_contains "$ROOT_DIR/README.md" \
  ...
  'the portability roadmap names DankMaterialShell without claiming support'
...
assert_block_contains "$card_geometry_block" \
  ...
  'the drawer fills the work-area-height native Side window'
assert_block_contains "$card_geometry_block" \
  'anchors.topMargin: 0' \
...
assert_contains "$ROOT_DIR/JotPin.qml" \
  ...
  'saving snapshots the active caret position'
assert_contains "$ROOT_DIR/JotPin.qml" \
  'String(editor.text || "") !== savedSource' \
  'saving never restores state over a newer edit'
assert_contains "$ROOT_DIR/JotPin.qml" \
  'editor.cursorPosition = Math.max(0,' \
  'saving preserves the active caret position'
assert_contains "$ROOT_DIR/JotPin.qml" \
