...
ShellRoot {
  ...
  Timer {
    ...
    onTriggered: {
      ...
      if (test.phase === 91 && test.mode === "session-prepare") {
        if (test.elapsed() < 350) return
        test.pass("closing JotPin left time for the session state write")
        test.finish()
        return
      }
      ...
      if (test.phase === 201 && test.mode === "tab-state-prepare") {
        if (pad.loadingFromFile || pad.notePath !== test.tabStateSecondPath ||
            pad.noteLoadedPath !== test.tabStateSecondPath) return
  ...
  Timer {
    ...
    onTriggered: test.fail("timed out in phase " + test.phase + " with " +
      (test.pad() ? test.pad().persistenceState() : "no component"))
  }
}
