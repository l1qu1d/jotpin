...
The complete headless suite uses Bash, Node.js, jq, ripgrep, Perl, the Lua compiler
...
services.
The opt-in live tests additionally use `grim`, `wtype`, `hyprctl`, and an
...
in a private HOME, a complete save/recovery lifecycle, and the real-QML renderer in
...
unchanged. With consent enabled, it verifies that the desktop entry supplies
JotPin in Apps and the menu extension adds `Personal > JotPin` without replacing
existing rows, that repeated deployment does not duplicate entries or binding requires, that a free `SUPER + N` receives the
default workspace-aware binding, and that an already-occupied or unverifiable
binding is left unchanged.

The cold-start regression (`bash tests/startup_regression.sh`) opens the production
component with a local SVG and highlighted code in a private offscreen process.
It checks readiness before reveal, stable initial layout, bounded stalled-image
waiting, terminal image errors, and the reveal latch during edits and Raw mode.
A stalled-parser case checks the delayed loading message and source recovery action.
It runs in the full headless suite and reports measured cold reveal latency.
...
the suite does not launch the user's browser. It also checks Settings dismissal,
the top-right version label following manifest metadata, the absence of the
Settings spellcheck section, text-size controls, and typing on the blank row after a checklist. The checklist
case verifies literal source insertion, unchanged row positions, and single-step undo.
Prefix Backspace checks cover bullets, numbers, tasks, headings, and quotes,
including preservation of the empty row and undo. The product harness also types
image Markdown, selects standalone and inline images, and drags all four handles
through the real resize and Undo handlers. Oversized images with following text
exercise wrapped object boundaries and width fitting. Image caption placement
and the introductory text staying above the image are checked during resizing. Inline-code typing colors are checked
both immediately and after parser reconciliation.
...
prevention are in the production-native parity gate. The retired
...
These focused offscreen checks cover the production Preview editor. The full
headless runner includes additional checks; see `tests/run.sh` for its complete
stage list:
...
## Persistence, settings, and session checks
...
fresh component restore. Values above the five-row limit are capped. Text size is clamped to 75% to
200%, rejects nonnumeric changes, and restores from the saved settings. The
Qt pointer harness verifies that an outside click closes Settings, an inside
click keeps it open, and text resizing updates both editors without changing
Markdown or dirty state.
...
is intentional. When a visual smoke test is needed, use:
...
The headless suite validates the plugin manifest, checks the source-preserving editor
bindings, and runs the CommonMark/GFM fixtures through the installed `md2html`
parser using disposable notes.
...
The `tab-state` focus runs per-tab selection, scroll, history persistence,
restart restoration, and stale-history rejection. It checks the persisted
first-tab selection before restart so a bad preparation cannot masquerade as
a load failure. The product keyboard harness also verifies that saving cannot
restore an older caret over a newer user selection.

Run the focused settings persistence check with:

```bash
bash tests/isolated_persistence_regression.sh settings
```

It verifies the saved preferences on disk and restores them in a new process.
Two text-size changes in the same event-loop turn also exercise a queued save:
each completion must expose its own value, and the final value must reach disk.
This catches duplicate completion signals for the first write even when the
second write happens to finish before the process exits.

Run the related completion-order checks with:

```bash
bash tests/isolated_persistence_regression.sh recovery-queued
bash tests/isolated_persistence_regression.sh close-completion
```

Queued recovery snapshots must report their respective source values and save
only the newest snapshot, without changing the note on disk. An unwritable
snapshot destination must preserve unsaved text and never report success.
Closing a saving or loading tab must load the replacement note without falling
back to the stalled-load retry, while preserving the affected files. These
checks run in the full headless suite and reproduce the original callback races
without depending on a slow machine.

Session-close verification waits for note, settings, and editor-state writes to
complete and checks the generated tab in the saved editor-state file. A fixed
sleep is not accepted as evidence that asynchronous writes have completed.

`bash tests/isolated_persistence_regression.sh dictionary-queued` adds two words
before the first dictionary write completes, checks the words in each completion,
and verifies that both reach disk. This check is also part of the full suite.

`bash tests/isolated_persistence_regression.sh recovery-cleanup-race` pauses an
actual recovery deletion with a fixture-only process barrier, then queues a new
snapshot before releasing it. The latest unsaved snapshot must survive on disk.
A second case saves the edit before releasing cleanup and requires the snapshot
to remain absent. The full suite includes both cases; ordinary timing cannot
skip the ordering under test.

`bash tests/isolated_persistence_regression.sh spellcheck-close` first delivers
accepted checked/suggestion responses through the production worker signal. It
then delivers the same responses after close and after reopen, requiring cleared
state to stay clear. This verifies generation invalidation without racing an
actual worker thread or waiting for a convenient scheduling order.

`bash tests/isolated_persistence_regression.sh terminal-reentry` forces another
state-machine tick immediately after a test finishes, before asynchronous Qt
exit can complete. A terminal latch must reject that tick. This prevents a
successful terminal step from running its edits twice and reporting a false
failure during shutdown.

The focused editor-command stage is available with
