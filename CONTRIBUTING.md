...
Thanks for helping improve JotPin. It is an on-demand Markdown scratchpad,
built for Omarchy first with shell-agnostic support as the long-term goal.
Its editor keeps Markdown source intact while rendering it in a native
...
```bash
git clone https://github.com/l1qu1d/jotpin.git
cd jotpin
```

JotPin runs on Omarchy with Quickshell. The headless test suite also uses the
tools listed in
...
For executable plugin, installer, or test-harness changes, run the complete
isolated suite before opening a pull request:

```bash
bash tests/run.sh headless
```

For documentation-only changes, check paths, commands, links, and consistency;
runtime tests and deployment are not required.
